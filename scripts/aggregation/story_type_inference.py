...
# v3.0.1 修复（T-029 P2-4）：去书名化——删除测试书专属词（上海堡垒的"泡防御/德尔塔/捕食者/防御罩"、
# 月亮与六便士的"画家/艺术家/艺术/理想主义/绘画/音乐/文学"），
# 避免对 T-004 金标准 20 部过拟合；词表只保留跨作品通用题材信号。
GENRE_KEYWORDS = {
    "科幻": ["科幻", "星际", "宇宙", "未来", "科技", "机器人", "AI", "人工智能", "外星", "太空",
             "文明", "星舰", "量子", "赛博", "末世", "末日",
             "外星人", "入侵", "能量", "飞船", "空间站", "虚拟", "赛博朋克", "量子", "宏原子", "球状闪电", "等离子", "高能物理", "实验装置", "粒子", "超导", "纳米", "基因", "克隆", "时空", "维度", "平行宇宙", "意识上传", "赛博"],
...
def infer_narrative_style(structure_rows: list[dict], interpretation_rows: list[dict]) -> dict:
    ...
    # 可靠性
    # v3.0.1 修复（T-029 P2-5）：无不可靠标注时不得默认声称"可靠"——
    # 无证据即返回 None（未知），而非 is_reliable=True
    if reliability_counter:
        is_reliable = not ("不可靠" in reliability_counter or "部分不可靠" in reliability_counter)
    else:
        is_reliable = None
