...
主要里程碑：
- **v3.9.0**：工程化加固（T-038/T-039/T-040，ADR-016）——①`span_locator.py` 模糊匹配增强（4级匹配：精确→空白归一→去标点→模糊相似度≥0.85，自动修正引文文本）；②`annotate_segment.py` 新增 `--auto-fix`/`--no-auto-fix`（craft层校验失败自动修复重试，默认开启）；③SKILL.md 新增 §4.0 枚举值速查表（15个维度全部枚举值一眼可查）。annotation/aggregation schema 不变（纯工程化加固）。
