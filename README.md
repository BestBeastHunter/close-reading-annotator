...
> **版本（决策 22 三域解耦）**：skill_version = `3.8.4` / annotation schema_version = `2.10.0` / aggregation schema_version = `3.1.0`。批注 JSON 向后兼容 `2.5.0`/`2.6.0`/`2.7.0`/`2.8.0`/`2.9.0`/`2.10.0`。
...
| 版本域 | 声明点 | 值 |
|--------|--------|-----|
| **skill version** | `SKILL.md` frontmatter `version` / README / RUNBOOK | `3.8.4` |
...
主要里程碑：
- **v3.8.4**：工程化加固（T-038/T-039/T-040，ADR-016）——①`span_locator.py` 模糊匹配增强（4级匹配：精确→空白归一→去标点→模糊相似度≥0.85，自动修正引文文本）；②`annotate_segment.py` 新增 `--auto-fix`/`--no-auto-fix`（craft层校验失败自动修复重试，默认开启）；③SKILL.md 新增 §4.0 枚举值速查表（15个维度全部枚举值一眼可查）。annotation/aggregation schema 不变（纯工程化加固）。
