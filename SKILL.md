...
> **版本声明（决策 22：三版本域解耦）**：
> - **skill version** = `3.16.1`（本文件 frontmatter = README = RUNBOOK）。最近变更：v3.16.1 发布前逐文件总检（T-144/T-145）——①修复聚合脚本 D19.target 同型 bug（character_network/character_biographies 改从 emotion 层顶层取 dict{name}）；②文档版本三域统一（skill 3.16.1 / annotation 2.10.0 / aggregation 3.5.0）；③批注深度策略修正：全量深度批注为默认且唯一正式档位，段采样分档仅显式降级。**完整版本历史见文末「版本历史」表。**
...
**批注管"逐段信号"，聚合管"全书拼图"**。聚合层把 L1-L4 批注（+D19/D15/D18 细粒度信号）组装成全局叙事图，供下游叙事分析 / 创作参考直接消费。10 个脚本纯规则零第三方依赖，全链路 <2s/本。
...
> ⚠️ **两套词表警告（高频踩坑）**：**D04.core（20 词）与 D19（50 词）是两套不同的词表**！`孤独` / `希望` / `压抑` / `释然` 等**只在 D04**（段落氛围），**不在 D19**（角色/精细情感）。写 D19 时用 D19 表，写 D04 时用 D04 表，混用会被校验器拒绝。
