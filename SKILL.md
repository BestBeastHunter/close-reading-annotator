...
>
> **v3.8.8 易用性提升**：新增 `examples/scene_boundary_wrapper.py` 官方 wrapper，一行命令就能跑场景边界判断，不需要自己写批量调用脚本。
...
> **v3.8.8 推荐方式**：直接用官方 wrapper 脚本，不需要自己写批量调用：
> ```bash
...
### 3.4.5 Phase 3.5：跨段关系 LLM 二分类精排（可选，v3.8.8 新增）

> **定位**：cross_segment.py 的规则候选（含 v3.8.8 新增的 D19.target 情感对象复用、D15 意象复用信号）召回率高但精度有限。本阶段用 Agent 自身 LLM 对规则候选做二分类精排，过滤误报，提升 cross_refs 质量。
